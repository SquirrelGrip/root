...
fun Any.toJson(): String =
    GsonEngine.get().toJson(this, this::class.java)

fun Any.toJson(writer: Writer) =
    GsonEngine.get().toJson(this, this::class.java, GsonEngine.get().newJsonWriter(writer))

fun Any.toJson(file: File) =
    this.toJson(file.toWriter())

fun Any.toJson(path: Path) =
    this.toJson(path.toWriter())

fun Any.toJson(outputStream: OutputStream) =
    this.toJson(outputStream.toWriter())

fun JsonElement.toJson() =
    GsonEngine.get().toJson(this)


inline fun <reified T> String.toInstance(): T =
    GsonEngine.get().fromJson<T>(this, T::class.java)

inline fun <reified T> Reader.toInstance(): T =
    GsonEngine.get().fromJson<T>(this, T::class.java)

inline fun <reified T> ByteArray.toInstance(): T =
    ByteArrayInputStream(this).toReader().toInstance<T>()

inline fun <reified T> InputStream.toInstance(): T =
    this.toReader().toInstance<T>()
...
inline fun <reified T> ByteArray.toInstance(
    ...
    this.copyOfRange(offset, offset + len).toInstance<T>()

inline fun <reified T> File.toInstance(): T =
    this.toReader().toInstance<T>()

inline fun <reified T> Path.toInstance(): T =
    this.toReader().toInstance<T>()


inline fun <reified T> String.toInstanceList(): List<T> =
    GsonEngine.get().fromJson<List<T>>(this, object : TypeToken<List<T>>() {}.type)

inline fun <reified T> Reader.toInstanceList(): List<T> =
    GsonEngine.get().fromJson<List<T>>(this, object : TypeToken<List<T>>() {}.type)

inline fun <reified T> InputStream.toInstanceList(): List<T> =
    this.toReader().toInstanceList<T>()
...
inline fun <reified T> ByteArray.toInstanceList(): List<T> =
    ByteArrayInputStream(this).toReader().toInstanceList<T>()

inline fun <reified T> ByteArray.toInstanceList(
    offset: Int,
    len: Int
): List<T> =
    this.copyOfRange(offset, offset + len).toInstanceList<T>()
...
inline fun <reified T> String.toJsonStream(): Stream<T> =
    StreamSupport.stream(Spliterators.spliteratorUnknownSize(GsonIterator(this, T::class.java), 0), false)

inline fun <reified T> Reader.toJsonStream(): Stream<T> =
    StreamSupport.stream(Spliterators.spliteratorUnknownSize(GsonIterator(this, T::class.java), 0), false)

inline fun <reified T> InputStream.toJsonStream(): Stream<T> =
    this.toReader().toJsonStream<T>()

inline fun <reified T> URL.toJsonStream(): Stream<T> =
    this.openStream().toJsonStream<T>()

inline fun <reified T> ByteArray.toJsonStream(): Stream<T> =
    ByteArrayInputStream(this).toJsonStream<T>()
...
fun Any.toJsonElement(): JsonElement =
    GsonEngine.get().toJsonTree(this, this::class.java)
