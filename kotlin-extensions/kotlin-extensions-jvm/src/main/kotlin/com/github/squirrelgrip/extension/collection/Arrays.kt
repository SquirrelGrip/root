...
class ArrayHandler<T, U>(
    ...
    private fun evaluate(
        predicate: (U) -> Boolean,
        item: T
    ): Boolean = predicate.invoke(transform.invoke(item))
...
): Boolean = ArrayHandler(expression, this, StringCompiler, aliases) { it }.all()

fun <T> Array<T>.allMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): Boolean = ArrayHandler(expression, this, StringCompiler, aliases, transform).all()
...
fun Array<String>.anyByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): Boolean = ArrayHandler(expression, this, StringCompiler, aliases) { it }.any()

fun <T> Array<T>.anyMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): Boolean = ArrayHandler(expression, this, StringCompiler, aliases, transform).any()

fun <T> Array<T>.anyFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): Boolean = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).any()

fun Array<String>.countByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): Int = ArrayHandler(expression, this, StringCompiler, aliases) { it }.count()

fun <T> Array<T>.countMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): Int = ArrayHandler(expression, this, StringCompiler, aliases, transform).count()

fun <T> Array<T>.countFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): Int = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).count()

fun Array<String>.filterByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): List<String> = ArrayHandler(expression, this, StringCompiler, aliases) { it }.filter()
...
fun Array<String>.filterNotByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): List<String> = ArrayHandler(expression, this, StringCompiler, aliases) { it }.filterNot()
...
fun Array<String>.findByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): String? = ArrayHandler(expression, this, StringCompiler, aliases) { it }.find()

fun <T> Array<T>.findMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): T? = ArrayHandler(expression, this, StringCompiler, aliases, transform).find()

fun <T> Array<T>.findFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): T? = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).find()

fun Array<String>.findLastByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
...
fun <T> Array<T>.findLastFlatMapByExpression(
    ...
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): T? = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).findLast()

fun Array<String>.firstByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): String = ArrayHandler(expression, this, StringCompiler, aliases) { it }.first()

fun <T> Array<T>.firstMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): T = ArrayHandler(expression, this, StringCompiler, aliases, transform).first()

fun <T> Array<T>.firstFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): T = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).first()
...
fun <T> Array<T>.firstOrNullMapByExpression(
    ...
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): T? = ArrayHandler(expression, this, StringCompiler, aliases, transform).firstOrNull()

fun <T> Array<T>.firstOrNullFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): T? = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).firstOrNull()
...
fun <T> Array<T>.indexOfFirstMapByExpression(
    ...
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): Int = ArrayHandler(expression, this, StringCompiler, aliases, transform).indexOfFirst()

fun <T> Array<T>.indexOfFirstFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): Int = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).indexOfFirst()

fun Array<String>.indexOfLastByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): Int = ArrayHandler(expression, this, StringCompiler, aliases) { it }.indexOfLast()

fun <T> Array<T>.indexOfLastMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): Int = ArrayHandler(expression, this, StringCompiler, aliases, transform).indexOfLast()

fun <T> Array<T>.indexOfLastFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): Int = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).indexOfLast()

fun Array<String>.lastByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): String = ArrayHandler(expression, this, StringCompiler, aliases) { it }.last()

fun <T> Array<T>.lastMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): T = ArrayHandler(expression, this, StringCompiler, aliases, transform).last()

fun <T> Array<T>.lastFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): T = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).last()

fun Array<String>.lastOrNullByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
...
fun <T> Array<T>.lastOrNullFlatMapByExpression(
    ...
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): T? = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).lastOrNull()

fun Array<String>.partitionByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap()
): Pair<List<String>, List<String>> = ArrayHandler(expression, this, StringCompiler, aliases) { it }.partition()

fun <T> Array<T>.partitionMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> String = { it.toString() }
): Pair<List<T>, List<T>> = ArrayHandler(expression, this, StringCompiler, aliases, transform).partition()

fun <T> Array<T>.partitionFlatMapByExpression(
    expression: String?,
    aliases: Map<String, String> = emptyMap(),
    transform: (T) -> Collection<String> = { setOf(it.toString()) }
): Pair<List<T>, List<T>> = ArrayHandler(expression, this, CollectionStringCompiler, aliases, transform).partition()
